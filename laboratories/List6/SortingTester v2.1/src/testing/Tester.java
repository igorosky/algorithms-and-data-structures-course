...
public class Tester {
	...
	public static <T> RunResult runOnce(AbstractSortingAlgorithm<MarkedValue<T>> algorithm,
		...
		timer.start();
		final List<MarkedValue<T>> sortedList = algorithm.sort(list);
		timer.stop();
		
		if(!ListChecker.isSorted(sortedList, algorithm.baseComparator())) {
		...
		return new RunResult(timer.durationMillis(), algorithm.comparisons(),
							 ListChecker.isSorted(sortedList, algorithm.baseComparator()),
							 ListChecker.isStable(sortedList, algorithm.baseComparator()));
	}
